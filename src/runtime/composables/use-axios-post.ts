 ...
 */
export const useAxiosPost = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, data?: D, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().post<T, R, D>(url, data, config)
}
