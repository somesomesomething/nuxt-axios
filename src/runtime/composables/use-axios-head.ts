 ...
 */
export const useAxiosHead = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().head<T, R, D>(url, config)
}
