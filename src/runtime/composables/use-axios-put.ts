 ...
 */
export const useAxiosPut = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, data?: D, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().put<T, R, D>(url, data, config)
}
