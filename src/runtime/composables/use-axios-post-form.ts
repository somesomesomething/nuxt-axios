 ...
 */
export const useAxiosPostForm = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, data?: D, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().postForm<T, R, D>(url, data, config)
}
