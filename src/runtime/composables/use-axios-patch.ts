 ...
 */
export const useAxiosPatch = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, data?: D, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().patch<T, R, D>(url, data, config)
}
