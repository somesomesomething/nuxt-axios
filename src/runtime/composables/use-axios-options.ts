 ...
 */
export const useAxiosOptions = <T = unknown, R = AxiosResponse<T>, D = unknown>(url: string, config?: NuxtAxiosRequestConfig<D>): Promise<R> => {
  return useAxios().options<T, R, D>(url, config)
}
